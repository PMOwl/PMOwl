...
<div class="replies panel panel-default list-panel replies-index" id="replies">
    ...
    <div class="panel-heading">
        <div class="total">{{ trans('Total Reply Count') }}: <b>{{ $replies->total() }}</b></div>
    ...
    <div class="panel-body">
        ...
        @if (count($replies))
            ...
            <div id="replies-empty-block" class="empty-block hide">{{ trans('No comments') }}~~</div>
        @else
            <ul class="list-group row"></ul>
            <div id="replies-empty-block" class="empty-block">{{ trans('No comments') }}~~</div>
    @endif
        ...
        <div class="form-group">
            @if ($currentUser)
                @if ($currentUser->verified)
                    <textarea class="form-control" rows="5" placeholder="{{ trans('Please using markdown.') }}"
                              style="overflow:hidden" id="reply_content" name="body" cols="50"></textarea>
                @else
                    <textarea class="form-control" disabled rows="5"
                              placeholder="{{ trans('You need to verify the email for commenting.') }}" name="body"
                              cols="50"></textarea>
                @endif
            @else
                <textarea class="form-control" disabled rows="5"
                          placeholder="{{ trans('User Login Required for commenting.') }}" name="body"
                          cols="50"></textarea>
            @endif
        </div>
